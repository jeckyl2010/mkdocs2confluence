...
### Nested Tuple Fix (v0.5.12) — Commit 333fff5 ✅ RESOLVED

**Change**: `ir/treeutil._rebuild` now handles `tuple[tuple[IRNode, ...], ...]` fields (e.g. `DefinitionItem.definitions`).

A third branch was added that detects outer tuples whose items are themselves IRNode tuples, and recurses into each inner tuple via `replace_nodes`. Previously, transforms (internal links, images, assets) silently skipped all content inside definition list definitions.

Regression test added in `tests/test_treeutil.py` — 9 tests now passing.
...
These three issues were identified and explicitly accepted by the user as known risks for this local CLI tool:
...
Roadmap items are future enhancements (GitHub Actions workflow, orphaned page detection) — not gaps in core functionality. Tool is **feature-complete** for its stated purpose.
